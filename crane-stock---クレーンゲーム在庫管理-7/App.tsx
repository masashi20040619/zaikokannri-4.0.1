  ...
  useEffect(() => {
    const initData = async () => {
      try {
        ...
        setSpendingRecords(spendingData);
      } catch (error) {
    ...
    setPrizes(prevPrizes => {
      const next = prevPrizes.map(p => {
        if (p.id === prizeId) {
          ...
          return { ...p, quantity: newQuantity, history: [...(p.history || []), historyEntry].slice(-100) };
        }
        return p;
      });
      setIsDirty(true);
    ...
    setPrizes(prevPrizes => {
      const next = prevPrizes.map(p => {
        if (p.id === prizeId && p.quantity > 0) {
          ...
          return { ...p, quantity: p.quantity - 1, history: [...(p.history || []), historyEntry].slice(-100) };
        }
        return p;
      });
      setIsDirty(true);
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 text-slate-800 dark:text-slate-200 font-sans pb-32">
      ...
      <main className="container mx-auto px-4 py-4 sm:py-6">
                ...
                allHistory.map((h, i) => (
                  <div key={i} className="bg-white dark:bg-slate-800 p-4 rounded-3xl border border-slate-100 dark:border-slate-700 flex items-start gap-4">
                    <div className={`mt-1 p-2 rounded-xl shrink-0 ${
                      h.displayType === 'sell' ? 'bg-rose-50 text-rose-500' :
                      h.displayType === 'buy' ? 'bg-emerald-50 text-emerald-500' :
                      h.displayType === 'spending' ? 'bg-amber-50 text-amber-500' :
                      'bg-slate-50 text-slate-400'
                    }`}>
                      {h.displayType === 'sell' ? <TrashIcon className="w-4 h-4" /> :
                    ...
                    <div className="flex-grow min-w-0">
                      <div className="flex justify-between items-start mb-1">
                        ...
                        {h.amount || h.price ? (
                          <p className={`text-sm font-black ${h.displayType === 'sell' ? 'text-emerald-600' : 'text-rose-600'}`}>
                            {h.displayType === 'sell' ? '+' : '-'}¥{(h.amount || (h.price * (h.quantity || 1))).toLocaleString()}
                          </p>
                        ) : null}
                      </div>
        ...
        {activeTab === 'data' && (
          <div className="animate-in fade-in slide-in-from-bottom-2 duration-300">
            <h2 className="text-xl font-black mb-6">データ管理</h2>
      ...
      <button
        onClick={() => { setPrizeToEdit(null); setIsModalOpen(true); }}
        className="fixed bottom-24 right-6 sm:bottom-28 sm:right-8 bg-indigo-600 text-white p-4 sm:p-5 rounded-2xl sm:rounded-3xl shadow-2xl shadow-indigo-300 transform transition-all hover:scale-110 active:scale-95 z-30"
      >
